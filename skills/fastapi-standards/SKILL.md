...
|---|---|
| settings | `app/core/config.py` — `class Settings(BaseSettings)`, `get_settings()` with `@lru_cache`, injected by `Depends(get_settings)`. Env names are fixed: `DATABASE_URL`, `REDIS_URL`, `SECRET_KEY`, `ENV` (`dev` / `test` / `prod`), `LOG_LEVEL`, `PORT`, `LLM_BASE_URL`; products add `TENANCY_MODE` (`saas` / `single`), `TENANT_ID` (single only), `AUTH_BACKEND` — an `internal` project omits them (`deployment-conventions` §Scope) |
| tenancy *(product)* | `get_tenant` in `deps.py` is the **only** reader of the tenant id: `saas` → the JWT `tid` claim, `single` → `settings.tenant_id`. Never from a body, query or header the client controls. Every service call receives the tenant from this dependency (`data-conventions` §Tenancy) |
| environment *(product)* | the app promises the same behaviour behind any edge — plain HTTP on `PORT`, proxy headers trusted only from the proxy, `/health` + `/ready`, graceful SIGTERM, no local state, JSON logs to stdout (`deployment-conventions`). Anything that needs the internet is behind a capability flag |
