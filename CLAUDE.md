...
- 사용자에게는 **항상 한국어로 답한다.** (개별 skill 에 이 지시를 반복하지 않는다 — 여기가 유일한 출처다)
- 코드·주석·커밋 메시지·문서 본문은 프로젝트의 기존 언어를 따른다.
...
- 규칙을 문서에 적는 것으로 끝내지 않는다. 린터·포매터·훅으로 옮길 수 있는 규칙은 반드시 옮긴다
  (`python-standards/templates/` 의 ruff·mypy 설정과 pre-commit 훅).
...
- 문서는 프로젝트 루트의 `docs/` 아래 2-depth 로 구조화한다
  (`concepts/`, `how-to/`, `reference/`, `design/`, `todo/`). 필요하면 3-depth 도 허용한다.
  **구조·작성 규칙의 단일 출처는 `docs-structure` skill** 이다 (Google 문서 관행 기반).
- 코드 변경과 문서 업데이트는 하나의 PR 로 묶는다. 나중에 갱신하는 문서는 갱신되지 않는 문서다.
- 기능 개발 완료 후:
  - 구현으로 확정된 사실은 `docs/concepts/`·`docs/reference/` 에 반영한다.
  - 설계 문서는 지우지 않고 **Status 를 `Implemented` 로 바꾼다** — "왜 이렇게 했나"에 답하는
    유일한 기록이다.
...
- Conventional Commits를 따른다. **허용 타입과 브랜치·릴리즈 절차의 단일 출처는 `git-workflow` skill** 이다
  (여기에 목록을 복사해두지 않는다 — 두 곳에 적으면 반드시 어긋난다).
- 1인 개발이라도 PR 프로세스를 유지한다. PR 생성까지가 AI 의 몫이고, 리뷰·머지는 사용자가 한다.
...
- skill 은 **결정**(규약·구조·워크플로)을 담는다. 라이브러리 **지식**은 skill 에 복사하지 않는다 —
  버전과 함께 썩기 때문이다. 설치된 버전의 소스(`site-packages`)와 MCP 문서에서 읽는다.
- skill 이 고정해도 되는 것은 **모양**(이름·URL·디렉토리·커밋·계약 위치)이고, **메커니즘**(어떤
